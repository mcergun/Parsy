...
public:
    ...
    uint64_t GrabU64(const char* buf, DataPosition pos);
    uint32_t GrabU32(const char *srcBuf, DataPosition pos);
    uint16_t GrabU16(const char *srcBuf, DataPosition pos);
    uint8_t GrabU8(const char *srcBuf, DataPosition pos);
    void GrabBuf(char *dstBuf, const char * srcBuf, DataPosition pos);
